...
## USE WITH CAUTION ** duplicate files on the target will be overwritten if they already exist. **

##define Azure storage account details
source_account_name = '**insert your source storage account name here**'
source_account_key = '**insert your source key here**'
source_account_sas = '**insert your source SAS token here**'

target_account_name = '**insert your target storage account name here**'
target_account_key = '**insert your target key here**'
...
target_account = CloudStorageAccount(target_account_name, target_account_key)
...
target_file_service = target_account.create_file_service()
    ...
    for file_or_dir in source_file_list: #interate through each file or directory in the current working directory
        ...
        if file_type == 'Directory': #if the current object is a directory, we are going to call this function again
            if current_dir == '': #if we are in the root folder of the share, we don't want to pass a forward slash to the function. '/dirname' is not valid
                target_file_service.create_directory(share.name, file_or_dir.name)
                ...
                target_file_service.create_directory(share.name, next_dir)
                list_file_and_dir(share, next_dir, nest_level + 1)
        else:
            if current_dir == '':
                source_file_path = source_file_service.make_file_url(share.name, None, file_or_dir.name)
                source_file_withtoken = source_file_path + source_account_sas
                copy = target_file_service.copy_file(share.name, None, file_or_dir.name, source_file_withtoken)
            else:
                source_file_path = source_file_service.make_file_url(share.name, current_dir, file_or_dir.name)
                source_file_withtoken = source_file_path + source_account_sas
                copy = target_file_service.copy_file(share.name, current_dir, file_or_dir.name, source_file_withtoken) 
...
for share in source_share_list:
    ...
    target_file_service.create_share(share.name)
